...
func TestUpgradeEntries(t *testing.T) {
	...
	require.Equal(t, int64(defines.MORPCVersion52), columns.RequiredProtocolVersion)
	...
	require.Equal(t, int64(defines.MORPCVersion52), hideInternalColumns.RequiredProtocolVersion)
	...
	for i, view := range metadataViews {
		...
		if view.name == "TABLES" {
			...
			expectedProtocol = defines.MORPCVersion52
		}
	...
	require.Equal(t, int64(defines.MORPCVersion52), columnsBinaryStrings.RequiredProtocolVersion)
	...
	require.Equal(t, int64(defines.MORPCVersion52), characterSetsUTF8Maxlen.RequiredProtocolVersion)
}
